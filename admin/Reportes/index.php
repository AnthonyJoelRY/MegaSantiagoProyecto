...
======================= -->
    ...
    <script>
        ...
        fetch(API + "?accion=ventasDia")
            ...
            .catch(err => {
                console.error("Error ventasDia:", err);
            });
...
====================== */
        fetch(API + "?accion=ventasMes")
            ...
            .catch(err => {
                console.error("Error ventasMes:", err);
                document.getElementById("tablaVentasMes").innerHTML = `
            <tr>
                <td colspan="2" class="text-danger text-center">
                    Error al cargar ventas por mes
                </td>
            </tr>
        `;
            });
        ...
        fetch(API + "?accion=clientesTop")
            ...
            .then(rows => {
                ...
                if (!rows || rows.length === 0) {
                    tbody.innerHTML = `
                <tr>
                    <td colspan="3" class="text-muted text-center">Sin datos</td>
                </tr>`;
                    return;
                }
